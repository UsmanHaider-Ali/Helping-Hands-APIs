...
exports.startProject = async (req, res, next) => {
  ...
  try {
    ...
    await contract.methods
      ...
      .send({ from: userAddress, gas: 611360 }, (error, transaction) => {
        if (!error) {
          res.json({
    ...
    for (let j = 0; j < result.length; j++) {
      ...
      for (let i = 0; i < rawMod.length; i++) {
        if (result[j][3] == rawMod[i]["projectId"]) {
          ...
          if (result[j][12] == 0) {
            status = "Pitched";
          } else if (result[j][12] == 1) {
            status = "Scheduled";
          } else if (result[j][12] == 2) {
            status = "Ongoing";
          } else if (result[j][12] == 3) {
            status = "Completed";
          } else if (result[j][12] == 4) {
            status = "Closed";
          }
